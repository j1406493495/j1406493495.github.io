...
background-image: https://i.loli.net/2019/06/15/5d04be26e8acc86507.jpg
date:  2018-11-05
category: Android
tags: 
- Android
...
- string 转换成 integer的方式及原理
  ...
  *答：*parseInt(s,10);第二个参数radix默认为十进制。

  > 1. 正常判断null，进制范围，length等
  > 2. 判断首字符是否为+-号，
  > 3. 循环遍历确定每个字符的十进制值
  > 4. 通过*= 和-= 进行计算拼接
  > 5. 判断是否为负值 返回结果
...
- 静态代理和动态代理的区别，什么场景使用？
  ...
  *答：动态代理运行时代理，静态代理代码层面实现代理。*

- Java的异常体系

  *答：Exception 和 Error 都继承自 Throwable 类。Exception 应该被捕获，绝大部分 Error 都会导致不可恢复，所以无需捕获。Exception 又可分为可检查和不检查。*
...
- JavaVM 和 JniEnv 的区别？
    ...
    *答：JavaVM 是 Java 在 JNI 层的代表，全局只有一个 JavaVM，封装一些对 JVM 操作的函数指针。JniEnv 是 Java线程的执行环境，一个线程对应一个 JniEnv。不同的 JniEnv 互不联通。*
